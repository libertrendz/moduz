 ...
 *      linha 1: logo (+ subtítulo no desktop)
 *      linha 2: dropdown + botões na MESMA linha (com wrap controlado)
 *  - Ajuste pontual adicional (padrão Moduz):
 *      - bolinha com cor por módulo nos botões de módulos ativos (nav)
 * =============================================
 */
...
type NavItem = { href: string; label: string; module_key: ModuleKey }

/**
 * Cores por módulo (UI) — usado apenas como identificação visual (bolinhas no nav).
...
function getModuleDotStyle(moduleKey: string): React.CSSProperties {
...
function buildMenu(keys: ModuleKey[]): NavItem[] {
  ...
  return sortedKeys
    .map((k) => ({
      module_key: k,
      href: ROUTES_BY_MODULE[k]?.href,
      label: ROUTES_BY_MODULE[k]?.label ?? k,
    }))
    .filter((x) => Boolean(x.href)) as NavItem[]
}
...
export function AdmShell(props: { children: React.ReactNode }) {
  ...
  const [menuItems, setMenuItems] = React.useState<NavItem[]>([
    { href: "/adm", label: "Core", module_key: "core" },
  ])
  ...
  async function loadContext() {
    ...
    try {
      ...
      if (!r.ok || !j || (j as any).ok === false) {
        ...
        setEmpresaId(null)
        setEnabledKeys(CORE_ONLY)
        setMenuItems([{ href: "/adm", label: "Core", module_key: "core" }])
        return
      }
              ...
              {menuItems.map((it) => (
                ...
                >
                  <span
                    ...
                    style={getModuleDotStyle(it.module_key)}
                  />
                  {it.label}
                </a>
              ))}
